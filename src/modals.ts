...
type CardKind = "quick-actions" | "modules" | "widgets";
...
/** Mobile-safe manager for dashboard-owned launchers and the global widget library. */
export class DashboardCardManagerModal extends Modal {
  ...
  onOpen(){
    ...
    const title=this.kind==="quick-actions"?"Manage quick actions":this.kind==="modules"?"Manage dashboard modules":"Manage dashboard widgets";
    const intro=this.kind==="quick-actions"?"Choose which shortcuts appear, set their order, and give important actions a visual treatment.":this.kind==="modules"?"Choose which connected add-ons appear, set their order, and give important modules a visual treatment.":"Choose which widgets appear on the dashboard. Hidden widgets stay available here with their saved layout, so you can add them back later.";
  ...
  private renderEntries(){
    ...
    if(!this.entries.length){list.createDiv({cls:"rbd-empty-state",text:this.kind==="modules"?"No connected modules are available.":this.kind==="widgets"?"No widgets are registered yet.":"No quick actions are configured."});return;}
    this.entries.forEach((entry,index)=>{
      const row=list.createDiv({cls:`rbd-card-manager-row${this.kind==="widgets"?" rbd-card-manager-widget-row":""}`});
  ...
  private async save(){
    ...
    else if(this.kind==="modules")this.entries.forEach((entry,index)=>{this.plugin.settings.moduleVisibility[entry.id]=entry.enabled;this.plugin.settings.moduleOrder[entry.id]=index+1;this.plugin.settings.moduleStyle[entry.id]=entry.style;this.plugin.settings.moduleFlavor[entry.id]=(entry.flavor??"").trim();});
    else {this.entries.forEach(entry=>{const layout=this.plugin.layout[entry.id];if(layout)layout.visible=entry.enabled;});await this.plugin.saveLayout();new Notice("Dashboard widgets updated.");this.close();return;}
